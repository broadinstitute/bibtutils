...
def test_bq_delete():
    ...
    bigquery.delete_table(TEST_PROJECT, TEST_DATASET, "test_delete_table")
...
def test_bigquery_create_table():
    ...
    bq.Client().delete_table(
        f"{TEST_PROJECT}.{TEST_DATASET}.test_create_table", not_found_ok=False
    )
...
def test_bigquery_create_and_upload():
    ...
    test_schema = [
        {"name": "test_name", "mode": "REQUIRED", "type": "STRING"},
        {"name": "test_desc", "mode": "NULLABLE", "type": "STRING"},
        {"name": "timestamp", "mode": "NULLABLE", "type": "INTEGER"},
        {"name": "upload_date", "mode": "NULLABLE", "type": "DATE"},
    ]
    ...
    client = bq.Client()
    client.delete_table(
        f"{TEST_PROJECT}.{TEST_DATASET}.test_create_upload_table", not_found_ok=True
    )
    ...
    # cleanup
    client.delete_table(
        f"{TEST_PROJECT}.{TEST_DATASET}.test_create_upload_table", not_found_ok=False
    )
