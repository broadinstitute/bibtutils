...
def retrigger_self(payload:str, proj_envar='_GOOGLE_PROJECT', topic_envar='_TRIGGER_TOPIC') -> None:
    '''
    Dispatches the next iteration of the function.

    Arguments:
        payload (dict): the pubsub payload.
...
def send_pubsub(topic_uri:str, payload:str) -> None:
    '''
    Publishes a pubsub message to the specified topic.
    Executing account must have pubsub publisher permissions on the topic or in the project.
...
def process_trigger(context, timeout_secs=1800, fail_alert_webhook_secret_uri='FAIL_ALERT_WEBHOOK_SECRET_URI') -> (str or None):
    '''
    Check timestamp of triggering event; catches infinite retry loops on 'retry on fail' cloud functions.
    If the timeout has been exceeded, will attempt to alert via Slack after fetching a webhook in Secret Manager whose name 
        should be provided in the environment variable specified in the function call. **It expects a full secret URI, not just a name!**
    If the triggering pubsub has a payload, will decode it as utf-8 and return it. Otherwise, will return None.
    ...
    Returns:
        str OR None: the pubsub payload, if present.
    '''
